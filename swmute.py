    ...
    async def swmutecmd(self, message):
        ...
        if message.chat:
            ...
            if not chat.admin_rights and not chat.creator:
                return await message.edit("<b>Я лох здесь.</b>")
            else:
                if chat.admin_rights.delete_messages == False:
                    return await message.edit("<b>У меня нет прав прав</b>")
        args = utils.get_args_raw(message)
        reply = await message.get_reply_message()
        chatid = str(message.chat_id)
        mutes = self.db.get("Mute", "mutes", {})
        if not args and not reply: return await message.edit("<b>Кого мутим?</b>")
        try:
            if args: 
